...
import com.gdms.crud.bean.User;
...
public class LoginController {
    ...
    @ResponseBody  
    public Msg Login(HttpSession httpSession,@ModelAttribute User user) {
		//用于存放错误信息
		String[] error_Msg = new String[3];
		//用于存放用户对象信息
		Object userInfo = null;
		//获取验证码图片中的验证码的值
        String vcCode = (String)httpSession.getAttribute("checkcode");          
        if(user.getUsername() == "")
        	error_Msg[0] = "请输入用户名";
        else if(user.getPassword() == "")
        	error_Msg[1] = "请输入用户密码"; 
        else if(user.getVCCode() == "")
        	error_Msg[2] = "请输入验证码"; 
        else {
        	//分三种权限进行判定
        	if("学生".equals(user.getRole()))
        		userInfo =  loginService.login(user.getUsername(), user.getPassword(), "学生");
        	else if("教师".equals(user.getRole())) 
        		userInfo =  loginService.login(user.getUsername(), user.getPassword(), "教师");
        	else if("管理员".equals(user.getRole())) 
        		userInfo =  loginService.login(user.getUsername(), user.getPassword(), "管理员");
        	if(userInfo == null)
                	error_Msg[1] = "用户名或密码错误";
        	else {
        		//System.out.println(vcCode.toUpperCase() + user.getVCCode().toUpperCase());
        		//测试时记得加回下面这一句代码
        		/*if(vcCode.toUpperCase().equals(user.getVCCode().toUpperCase())) {*/
        		if(!vcCode.toUpperCase().equals(user.getVCCode().toUpperCase())) {
                	error_Msg[2] = "验证码错误";
        		}else {
            		//用户输入的验证码正确后，将查询出的对象转换为对应的类型并放进session会话中
            		if("学生".equals(user.getRole())) {
	 ...
	 * 功能：生成验证码
	 */
	@RequestMapping(value="Generate_vcCode",method=RequestMethod.GET)  
    @ResponseBody     
	public Msg Generate_verification_code(HttpSession httpSession,HttpServletRequest request, HttpServletResponse response) throws IOException{
	    int width = 120;	//生成图片的宽度  
        int height = 30;  //生成图片的高度
        ...
        for (int i = 0; i < 4; i++) {  
            // 随机颜色  
            graphics2d.setColor(new Color(0 + random.nextInt(110),0 + random  
                    .nextInt(110), 0 + random.nextInt(110)));  
            // 旋转 -30 --- 30度  
        ...
        // 将生成的字母存入到session中  
        httpSession.setAttribute("checkcode", sb.toString());  
