	...
	$.ajax({
		...
		success:function(result){
			...
			$.each(result.extend.auditstatustypelist,function(){
				var optionAuditStatus = $("<option></option>").append(this.auditstatusname).attr("value",this.auditstatusid);
				optionAuditStatus.appendTo(ele3);
			});				
		}
	});	
}
...
function build_GD_table(result){
	...
	$.each(gds,function(index,item){
		if(item.ktbginfo != null){
		var ktbgInfoidTd = $("<td></td>").append(item.ktbginfo.ktbginfoid);
		...
		var auditstatusTd =$("<td></td>").append(item.ktbgauditstatus.auditstatusname);
		var uploadtimeTd = $("<td></td>").append(item.ktbginfo.uploadtime);
		if(item.student != null){
			var studentIdTd = $("<td></td>").append(item.student.name + "(" + item.studentid + ")");	
			}else{
			var studentIdTd = $("<td></td>").append("");
			}	
		...
		var auditBtn = $("<button></button>").addClass("btn btn-danger audit_btn").append($("<span></span>").addClass("glyphicon glyphicon-tags")).append("审核");
		//为修改按钮添加一个自定义的属性来表示当前毕设的编号
		auditBtn.attr("auditid",item.graduationdesignid);		
		
		//append方法执行完成以后还是返回原来的元素
		$("<tr></tr>").append(ktbgInfoidTd)
		.append(graduationdesignresultidTd)
...
function to_page(pn){
		...
		$.ajax({
			url:"QueryTeacherGDResultlist",
			data:{"xttm":xttm,"tmlx":tmlx,"tmly":tmly,"shzt":shzt,"pn":pn},
			type:"POST",
			success:function(result){
				//console.log(result);
				//1、解析并显示用户数据
...
$(function(){
	...
	var ktbgInfoId = $(this).parents("tr").find("td:eq(0)").text();
	var gdResultID = $(this).parents("tr").find("td:eq(1)").text();
	var gdID = $(this).parents("tr").find("td:eq(2)").text();
	window.location.href = "JumpKTBGDetailPages?gdResultID="+ gdResultID + "&gdID=" + gdID +"&ktbgInfoId=" + ktbgInfoId;				
});
});
...
$(function(){
	...
	var gdID = $(this).parents("tr").find("td:eq(2)").text();
	var gdResultID = $(this).parents("tr").find("td:eq(1)").text();
	var shzt = $(this).parents("tr").find("td:eq(6)").text();
	if(shzt == "等待指导老师审核"){
		window.location.href = "TeacherJumpAuditKTBGPages?gdID="+gdID + "&gdResultID=" + gdResultID+ "&ktbgInfoid=" + ktbgInfoid;
	}else{
		alert("你已对该毕设进行了审核");
	}				
});
