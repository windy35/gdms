...
function build_GD_table(result){
	...
	$.each(gds,function(index,item){
		...
		var declaretimeTd = $("<td></td>").append(item.graduationdesign.declaretime);
...
$(function(){
	...
	var gdID = $(this).parents("tr").find("td:eq(1)").text();
	var gdResultID = $(this).parents("tr").find("td:eq(0)").text();
	var shzt = $(this).parents("tr").find("td:eq(5)").text();
	if(shzt == "等待指导老师审核"){
		window.location.href = "JumpAuditBSXTPages?gdID="+gdID + "&gdResultID=" + gdResultID;
