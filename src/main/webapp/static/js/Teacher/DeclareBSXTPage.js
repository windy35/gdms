...
function JumpDeclareGD(){
	window.location.href="TeacherJumpDeclareBSXTSuccess";
/*	$.ajax({
		url:"TeacherJumpDeclareGD",
		type:"GET",		
		success:function(result){
			//console.log(result);
			if(result.code == 100){
				window.location.href="TeacherJumpDeclareGDSuccess";
			}else{
				alert("无法重复申报毕设选题");
			}
		},		
...
function to_page(pn){
		...
		$.ajax({
			url:"QueryTeacherOwnGDResultlist",
...
$(function(){
	...
	window.location.href = "JumpBSXTDetailPages?gdID="+gdID+"&gdResultID=" + gdResultID;			
});
});
...
$(function(){
	...
	var gdResultID = $(this).parents("tr").find("td:eq(0)").text();
	var gdID = $(this).parents("tr").find("td:eq(1)").text();
	window.location.href = "TeacherJumpAlertGDPages?gdID="+gdID+"&gdResultID=" + gdResultID;
});
});
