...
//获取字数
$(function(){	
	if(document.getElementById("ktbgdetail") != null){
		var ktbgDetailZs = document.getElementById("ktbgdetail").value.length;
		$("#ktbgDetailzs").append(ktbgDetailZs);
	$('#ktbgdetail').bind('input',function(){  
		ktbgDetailZs = document.getElementById("ktbgdetail").value.length;
		$("#ktbgDetailzs").empty();
		$("#ktbgDetailzs").append(ktbgDetailZs);
		});		
	}	
	$("#returnBtn").click(function(){
		window.location.href="JumpUploadKTBGPage";
	})	
});
...
function build_KTBG_table(result){	
	//清空table表格中的数据
	$("#KTBGTable tbody").empty();
	...
	$.each(gds,function(index,item){
		...
		if(item.ktbgauditstatus != null)
		var ktbgauditstatusnameTd = $("<td></td>").append(item.ktbgauditstatus.auditstatusname);
		...
		var ktbgInfoHidden =  $("<td></td>").append($("<input></input>").attr("type","hidden").attr("value",item.ktbginfoid).attr("id","ktbgInfoHidden"));
		...
		.append(ktbgauditstatusnameTd)
		.append(uploadtimeTd)
		.append(checkBtn)
		.append(updateBtn)
		.append(gdResultIdHidden)
		.append(ktbgInfoHidden)
		.appendTo("#KTBGTable tbody");
	});			
}
...
$(function(){
	$(document).on("click",".check_btn",function(){		
		...
		var ktbgInfoId = $("#ktbgInfoHidden").val();
		var gdResultID = $("#gdResultIdHidden").val();
		//console.log(gdResultID + " % " + KtbgInfoid + " % " +  gdResultID);
		window.location.href = "JumpKTBGDetailPages?gdID="+gdID+"&gdResultID=" + gdResultID+"&ktbgInfoId=" + ktbgInfoId;
});
});
...
$(function(){
	$(document).on("click",".update_btn",function(){
		var ktbgInfoId = $("#ktbgInfoHidden").val();
		window.location.href="JumpAlertKTBGPage?ktbgInfoId=" + ktbgInfoId;
});
});
...
$(function(){
	$("#submitBtn").click(function(){
		reset_form("#ktbgForm");
		var formData = new FormData(document.getElementById("ktbgForm")); 
		$.ajax({
			url:"STUalertKTBG",
			data:formData,
			type:"POST",
			...
			success:function(result){
				if(result.code == 100){
					...
					if(result.extend.error_Msg[0] != null){
						show_validate_msg("#ktbgdetail","error",result.extend.error_Msg[0]);
					}
				}
			}
			});
