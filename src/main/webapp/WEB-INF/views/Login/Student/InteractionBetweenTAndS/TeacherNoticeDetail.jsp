...
<!-- 自定义js文件 -->
	<script type="text/javascript" src="${APP_PATH}/static/js/Teacher/TeacherNoticeDetail.js"></script>    
<!--网页小图标-->
...
<a href="JumpTeacherNoticeDetail?noticeid=${requestScope.PreviewDetail.noticeid}">${requestScope.PreviewDetail.noticetitle}</a>
</c:if>
</div>
<div>
下一条：
<c:if test="${not empty requestScope.hasNext}">
<a href="JumpTeacherNoticeDetail?noticeid=${requestScope.NextDetail.noticeid}">${requestScope.NextDetail.noticetitle}</a>
</c:if>
</div>
<br>
<div>
	<input type="button" value="返回" class="btn btn-primary" id="returnMoreDetailBtn">	
</div>
</body>
</html>