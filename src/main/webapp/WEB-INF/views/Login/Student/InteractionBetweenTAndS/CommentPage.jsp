...
<!-- 自定义js文件 -->
    ...
    <script type="text/javascript" src="${APP_PATH}/static/js/Student/CommentPage.js"></script>
<!--网页小图标-->
...
<tr>
	<td>
		...
		<c:if test="${fn:contains(list.dianzanzheno, sessionScope.user.studentid) == false}">
			<span class="heart" rel="like" style="background-position: left center;"></span>
		</c:if>
		<c:if test="${fn:contains(list.dianzanzheno,sessionScope.user.studentid)}">
		...
		<div class="row">
			...
			<div class="col-md-6">
				<nav aria-label="Page navigation">
				  <ul class="pagination">
				  	<li><a href="${APP_PATH}/StuJumpCommentPage?titleid=${lyhdtitleinfo.lyhdtitleinfoid}&pn=1">首页</a></li>
				    <li>
				    <!-- 上一页 -->
				    <c:if test="${pageInfo.hasPreviousPage == true }">
				      	<a href="${APP_PATH}/StuJumpCommentPage?titleid=${lyhdtitleinfo.lyhdtitleinfoid}&pn=${pageInfo.pageNum-1}" aria-label="Previous">
				    ...
				    <!-- 如果遍历到的页码为当前页面，则高亮显示 -->
				    	<c:if test="${page_Num == pageInfo.pageNum}">
				    		<li class="active"><a href="${APP_PATH}/StuJumpCommentPage?titleid=${lyhdtitleinfo.lyhdtitleinfoid}&pn=${page_Num}">${page_Num}</a></li>
				    	</c:if>
				    	<c:if test="${page_Num != pageInfo.pageNum}">
				    		<li><a href="${APP_PATH}/StuJumpCommentPage?titleid=${lyhdtitleinfo.lyhdtitleinfoid}&pn=${page_Num}">${page_Num}</a></li>
				    	</c:if>
				    </c:forEach>				    
				    <li>
				     <c:if test="${pageInfo.hasNextPage == true }">
				      <a href="${APP_PATH}/StuJumpCommentPage?titleid=${lyhdtitleinfo.lyhdtitleinfoid}&pn=${pageInfo.pageNum+1}" aria-label="Next">
				        <span aria-hidden="true">&raquo;</span>
				      </a>
				      </c:if>
				    </li>
				    <li><a href="${APP_PATH}/StuJumpCommentPage?titleid=${lyhdtitleinfo.lyhdtitleinfoid}&pn=${pageInfo.pages}">尾页</a></li>
				  </ul>
				</nav>				
			</div>	
		</div>
